...
## V4 zero accent — 2026-09-11

Added the user's short rising inner stroke to 0, joining its inner left wall and tapering before the right wall. The reference JPEG is preserved unchanged as `zero-accent-reference.jpg`. `zero-accent.ts` authors the vector, samples the existing zero face color and bakes its narrow bevel under the same three light positions. This adds no image-model generation and leaves the original TTF untouched. V4 metrics/provenance include the accent path.

All six atlases compare pixel-identically with v3 outside the zero tile, including O. Existing opaque zero pixels, glyph metrics, kerning and optical placements are unchanged. Each zero adds 1,022 pixels at the 50% alpha threshold; no existing mask pixels are removed, and the right gap remains open. Every lighting frame and palette has identical alpha. Reviewed 0/O at 32, 56, 107 and 330 px and all three edge lights. Font/export checks, lite/full HUD smoke checks and build passed without browser errors. The full suite was not run. Local validation took approximately 24 minutes from 2026-09-10 23:05:27 UTC; publication verification follows.

## V5 zero rebuild — 2026-09-11

The user rejected v4's thin accent and required starting from the flat letterform, making the accent substantially broader, and rerunning the whole zero through the full pipeline. `build-zero-source.mjs` now modifies the original inner counter path before any rendering. The broad root and tapered stroke are part of one continuous glyph contour. A fresh built-in image-model pass uses the black-and-white PNG as authoritative shape and the original gold zero as material reference; its raw RGBA output, exact prompt and both input hashes are preserved. Removed the separate accent shader/compositing code. The complete revised vector runs through the standard bevel mesh, material projection, analytic alpha and three lighting bakes in both palettes.

The accent now occupies 3,029 mask pixels versus v4's 1,022, while the right gap remains open. All pixels outside zero, all metrics and the original outer contour are unchanged. The whole zero's material was refreshed, including the ring. All frames/palettes have identical alpha and no gray/magenta contamination. Reviewed 32–330 px, all three lights, exports and lite/full HUD; focused checks and build pass. No full suite run. Approximately 10 minutes from 2026-09-10 23:48:37 UTC to local verification; publication checks follow. Sources and prompt: `zero-v5/README.md`.
