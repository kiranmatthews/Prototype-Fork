...
export const rooMenuPalette=(node:Element)=>node.closest('.game-logo,.game-menu-button:not(.game-control-hint)')?'counter' as const:rooMenuTitle(node)||!!node.closest('.timber-card')?'bonus' as const:'counter' as const;
