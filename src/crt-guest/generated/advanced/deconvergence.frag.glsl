...
// WebGL2 translation of advanced/deconvergence.slang
// libretro/slang-shaders @ a62d9cda9140294d22b6da5e4ff4187365890d42
// Source SHA-256: 0a94521bfe23ee6c2bd67986260d7671619258e0975689e98be565cbad0e11c5
...
const float _1871[15] = float[](2.0, 3.0, 3.0, 6.0, 6.0, 2.400000095367431640625, 3.400000095367431640625, 2.400000095367431640625, 3.25, 3.400000095367431640625, 4.400000095367431640625, 4.25, 7.400000095367431640625, 6.25, 5.25);
...
void fetch_pixel(inout highp vec3 c, inout highp vec3 b, inout highp vec3 g, highp vec2 coord, highp vec2 bcoord)
{
    ...
    d = vec3(r1, g1, b1);
    highp vec3 _1423 = mix(b, d, vec3(min(ds, 1.0)));
    g = _1423;
    b = _1423;
...
void main()
{
    ...
    bool _1482 = intera < 0.3499999940395355224609375;
    bool _1489;
    if (_1482)
    {
        _1489 = uGlobal_interm == 6.0;
    }
    else
    {
        _1489 = _1482;
    ...
    bool inter6 = _1489;
    bool _1495 = (intera < 0.3499999940395355224609375) && (!inter6);
    bool _1503;
    if (!_1495)
    {
        _1503 = uGlobal_no_scanlines > 0.02500000037252902984619140625;
    }
    else
    {
        _1503 = _1495;
    ...
    if ((uParams_IOS > 0.0) && (!interb))
    {
        ...
        highp vec2 _1568 = Overscan(param, param_1, param_2);
        texcoord = _1568;
        bool _1571 = uParams_IOS == 1.0;
        bool _1578;
        if (!_1571)
        {
            _1578 = uParams_IOS == 3.0;
        }
        else
        {
            _1578 = _1571;
    ...
    highp vec2 _1619 = Overscan(param_3, param_4, param_5);
    texcoord = _1619;
    ...
    highp vec2 _1651 = Overscan(param_6, param_7, param_8);
    texcoord = _1651;
    highp vec2 pos1 = vTexCoord;
    highp vec2 param_9 = texcoord;
    highp vec2 _1657 = Warp(param_9);
    highp vec2 pos = _1657;
    highp vec2 param_10 = vTexCoord;
    highp vec2 _1661 = Warp(param_10);
    highp vec2 pos0 = _1661;
    highp vec2 posb = (pos0 - vec2(0.5)) * 2.0;
    if (uParams_BLOOM < 0.5)
    {
    ...
    {
        highp vec3 param_11 = color;
        highp vec3 param_12 = Bloom;
        highp vec3 param_13 = Glow;
        highp vec2 param_14 = pos1;
        highp vec2 param_15 = pos;
        fetch_pixel(param_11, param_12, param_13, param_14, param_15);
        color = param_11;
        Bloom = param_12;
        Glow = param_13;
    }
    ...
    highp vec2 dx = mix(vec2(0.001000000047497451305389404296875, 0.0), vec2(0.0, 0.001000000047497451305389404296875), vec2(uParams_TATE));
    ...
    dx = mix(vec2(uGlobal_OriginalSize.z, 0.0), vec2(0.0, uGlobal_OriginalSize.w), vec2(uParams_TATE)) * 0.25;
    ...
    highp float mwidth = _1871[int(max(uParams_shadowMask, 0.0))];
    highp float mask_compensate = fract(mwidth);
    highp float mwidth1 = mwidth;
    if (uParams_shadowMask > (-0.5))
    {
        highp vec2 maskcoord = (vTexCoord * uGlobal_OutputSize.xy).yx;
        if (notate)
        {
        ...
        if (!zoomed)
        {
            highp vec2 param_17 = floor(maskcoord);
            highp float param_18 = mx;
            highp float param_19 = mb;
            highp vec3 _1978 = Mask(param_17, param_18, param_19);
            cmask = _1978;
        }
        else
        {
            ...
            bool _2013 = uParams_shadowMask == 12.0;
            bool _2020;
            if (_2013)
            {
                _2020 = uGlobal_mask_zoom == (-2.0);
            }
            else
            {
                _2020 = _2013;
            ...
            highp vec2 param_20 = vec2(mcoord, maskcoord.y);
            highp float param_21 = mx;
            highp float param_22 = mb;
            highp vec3 _2037 = Mask(param_20, param_21, param_22);
            highp vec2 param_23 = vec2(mcoord + 1.0, maskcoord.y);
            highp float param_24 = mx;
            highp float param_25 = mb;
            highp vec3 _2048 = Mask(param_23, param_24, param_25);
            cmask = mix(_2037, _2048, vec3(mlerp));
        }
        highp float sm_offset = 0.0;
        bool _2056 = uParams_shadowMask == 0.0;
        bool _2063;
        if (!_2056)
        {
            _2063 = uParams_shadowMask == 2.0;
        }
        else
        {
            _2063 = _2056;
        ...
        if (!_2084)
        {
            _2091 = uParams_shadowMask == 11.0;
        }
        else
        {
            _2091 = _2084;
        }
        bool bsm_offset = _2091;
        if (zoomed)
        {
        ...
        highp vec2 param_26 = scoord + vec2(sm_offset, 0.0);
        highp float param_27 = mx;
        highp float param_28 = swidth;
        highp float _2126 = SlotMask(param_26, param_27, param_28);
        smask = _2126;
        ...
        if (abs(uParams_mask_bloom) > 0.02500000037252902984619140625)
        {
            ...
            {
                highp vec3 param_29 = pow(Bloom, vec3(0.3499999940395355224609375));
                highp float param_30 = maxbl;
                cmask = max(mix(cmask, (cmask * (1.0 - (0.5 * maxbl))) + plant(param_29, param_30), vec3(-uParams_mask_bloom)), cmask);
            }
        }
    ...
    if (uParams_pr_scan > 0.02500000037252902984619140625)
    {
        ...
        highp vec3 param_31 = orig1;
        highp float param_32 = mbl;
        Bloom = mix(Bloom, mix(Bloom, plant(param_31, param_32), vec3(min(2.5 * (1.0 - w3), 1.0))), vec3(min(2.0 * uParams_pr_scan, 1.0)));
    }
    ...
    if (abs(uGlobal_bloom) > 0.02500000037252902984619140625)
    {
        ...
        {
            highp vec3 param_33 = Bloom;
            highp float param_34 = maxb;
            Bloom1 = plant(param_33, param_34);
        }
    ...
    if (uGlobal_halation > 0.00999999977648258209228515625)
    {
        ...
        highp vec3 param_35 = Bloom;
        highp float param_36 = mix(sqrt(mbl_1 * cmxh), max(mbl_1 - (0.1500000059604644775390625 * (1.0 - colmx)), 0.4000000059604644775390625 * cmxh), pow(colmx, 0.25));
        Bloom = plant(param_35, param_36) * mix(0.425000011920928955078125, 1.0, colmx);
        highp vec3 param_37 = vec3(0.324999988079071044921875) + (orig1 / vec3(w3));
        highp float param_38 = 0.5 * (1.0 + w3);
        Bloom = (((vec3(3.0 - colmx) - color) * plant(param_37, param_38)) * hcmask) * Bloom;
    ...
    else
    {
        if (uGlobal_halation < (-0.00999999977648258209228515625))
        {
            ...
            highp vec3 param_39 = ((Bloom + Ref) + orig1) + ((Bloom * Bloom) * Bloom);
            highp float param_40 = min(mbl_2 * mbl_2, 0.75);
            Bloom = plant(param_39, param_40);
            Bloom = (hcmask * (2.0 * mix(1.0, w3, 0.5 * colmx))) * Bloom;
            color -= (Bloom * uGlobal_halation);
        }
    }
    color = min(color, vec3(1.0));
    highp vec3 param_41 = color;
    highp float param_42 = w3;
    color = gc2(param_41, param_42);
    if (uGlobal_smoothmask > 0.125)
    {
        ...
        highp vec3 param_43 = orig1;
        highp float param_44 = 1.0 + ((0.17499999701976776123046875 * colmx) * uGlobal_smoothmask);
        color = mix(min(color / vec3(w4), plant(param_43, param_44)) * w4, color, vec3(w4));
    }
    ...
    else
    {
        ...
        highp vec3 param_45 = orig1 + (Ref * 0.001000000047497451305389404296875);
        highp float param_46 = 1.0;
        highp vec3 orig2 = plant(param_45, param_46);
        highp vec3 param_47 = Glow;
        highp float param_48 = 1.0;
        Bloom = plant(param_47, param_48);
        Ref = abs(orig2 - Bloom);
    ...
    {
        highp vec3 param_49 = vec3(floor((uGlobal_OutputSize.xy * vTexCoord) / vec2(uGlobal_noiseresd)), float(uGlobal_FrameCount));
        highp vec3 _3029 = _noise(param_49);
        highp vec3 noise0 = _3029;
    ...
    highp float param_50 = mix(pos.y, pos.x, uGlobal_bardir);
    highp float _3097 = humbar(param_50);
    highp vec2 param_51 = posb;
    highp float _3105 = corner(param_51);
    FragColor = mix(vec4((((color * vig) * _3097) * uGlobal_post_br) * _3105, 1.0), crtGuestSampleLinearBorder(StockPass, (floor(pos1 * uGlobal_OriginalSize.xy) + vec2(0.5)) * uGlobal_OriginalSize.zw, 0.0), vec4(step(pos1.x, uGlobal_oimage - 0.00025000001187436282634735107421875)));
}
