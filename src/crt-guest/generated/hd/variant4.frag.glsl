...
// WebGL2 translation of hd/linearize-hd.slang
// libretro/slang-shaders @ a62d9cda9140294d22b6da5e4ff4187365890d42
// Source SHA-256: 35c24c494dc6ba38f64386976c7e56421d161dd1e398a73ece2ecdba708ce659
...
void main()
{
    highp vec3 c1 = crtGuestSampleLinearBorder(Source, vTexCoord, 0.0).xyz;
    highp vec3 c2 = crtGuestSampleLinearBorder(Source, vTexCoord + vec2(0.0, uParams_SourceSize.w), 0.0).xyz;
    highp vec3 c = c1;
    ...
    bool _160 = uParams_inter <= (uParams_OriginalSize.y / yres_div);
    bool _166;
    if (_160)
    {
        _166 = uParams_interm > 0.5;
    }
    else
    {
        _166 = _160;
    ...
    if (_185 || hscan)
    {
        ...
        bool _216 = uParams_interm < 3.5;
        bool _224;
        if (!_216)
        {
            _224 = uParams_interm > 4.5;
        }
        else
        {
            _224 = _216;
        ...
        if (_224)
        {
            ...
            {
                highp vec3 param = mix(c2, c2 * c2, vec3(uParams_iscans));
                highp float param_1 = max(max(c2.x, c2.y), c2.z);
                c2 = plant(param, param_1);
                r = max(m1 * ii, (1.0 - uParams_iscan) * min(m1, m2));
                highp vec3 param_2 = mix(mix(c1, c2, vec3(min(mix(m1, 1.0 - m2, min(m1, 1.0 - m1)) / (d + 9.9999997473787516355514526367188e-06), 1.0))), c1, vec3(ii));
                highp float param_3 = r;
                c = plant(param_2, param_3);
        ...
        {
            highp vec3 param_4 = mix(c, c * c, vec3(0.5 * uParams_iscans));
            highp float param_5 = max(max(c.x, c.y), c.z);
            c = plant(param_4, param_5) * (1.0 - (0.5 * uParams_iscan));
        }
        if (hscan)
        {
            c = c1;
