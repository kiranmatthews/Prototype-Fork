...
// WebGL2 translation of hd/bloom_horizontal.slang
// libretro/slang-shaders @ a62d9cda9140294d22b6da5e4ff4187365890d42
// Source SHA-256: 417824f3056a57067ad57b6bb973aa321450fd3033f5c39b375d1e478e2bb074
...
uniform highp float uParams_SIZEHB;
uniform highp float uParams_SIGMA_HB;
uniform highp float uParams_FINE_BLOOM;
uniform highp float uParams_auto_res;
uniform highp float uParams_GLOW_MAX;
...
void main()
{
    ...
    {
        _28 = mix(1.0, 2.0, clamp((uParams_auto_res * round(uParams_OriginalSize.x / 300.0)) - 1.0, 0.0, 1.0));
    }
    else
    {
        _28 = 1.0;
    ...
    {
        _55 = uParams_FINE_BLOOM;
    }
    else
    {
        _55 = mix(0.75, 0.5, -uParams_FINE_BLOOM);
    }
    FINE_BLOOM = _55;
    invsqrsigma = 1.0 / ((((2.0 * uParams_SIGMA_HB) * uParams_SIGMA_HB) * auto_rez) * auto_rez);
    highp vec4 SourceSize1 = uParams_OriginalSize * vec4(FINE_BLOOM, FINE_BLOOM, 1.0 / FINE_BLOOM, 1.0 / FINE_BLOOM);
    ...
    highp float n = (-uParams_SIZEHB) * auto_rez;
    for (int crtGuestLoop0 = 0; crtGuestLoop0 < 512; crtGuestLoop0++)
    {
        highp vec4 pixel = crtGuestSampleLinearBorder(LinearizePass, tex + (dx * n), 0.0);
