...
import { skateMethodElasticity, skateUnderRailElasticity, skate900Elasticity, skateBackflipElasticity, skateFootFlipElasticity, skateImpossibleElasticity, skateRevertElasticity, SKATE_UNDER_RAIL_ARM_LIMIT } from './animation/elasticity';
...
const _trickOffset = new THREE.Vector3();
const _trickPivot = new THREE.Vector3();
const _trickRight = new THREE.Vector3(1,0,0);
const _trickFrameQ = new THREE.Quaternion();
const _trickRotationQ = new THREE.Quaternion();
...
interface GroundHit {
  ...
  mesh?: THREE.Object3D; // exact mesh contact for swept transition attachment
...
export class Player {
  ...
  private balanceEntryAge = 0; // current catch only; never resets combo difficulty
  ...
  captureRunState(): PlayerRunState {
    return {
      ...
      trickHistory: [...this.runTrickUses],
    };
  }
  ...
  private grabPose = 0;
  private grabPitch = 0; // smoothed parameters of the latched grab pose
  private grabRoll = 0;
  private armRPose = 0;
  private armLPose = 0;
 ...
 // what this air's grab actually paid — repriced when the variant resolves to a different trick's decay pool
  private comboUses = new Map<string, number>();
  private runTrickUses = new Map<string, number>();
  private heldTrickScores = new Map<'grab'|'grind'|'manual'|'lip'|'wallride', HeldTrickScore>();
  ...
  private flipT = 0;
  private queuedFlip: {x:number;y:number;time:number;special:SpecialTrick|null} | null = null;
  private queuedGrab: {kind:GrabTrickKind;time:number;special:SpecialTrick|null} | null = null;
  private grabBlockedUntilRelease = false;
  private grabKind: GrabTrickKind = 'indy';
  private deckYawOffset = 0; // deck flip trick in progress: time left of CONST.flipTime
  ...
  private flipDuration: number = CONST.flipTime;
  ...
  get skateCameraUp(): Readonly<THREE.Vector3> {
    // Match syncVisual's quaternion slerp, including partially level airs.
    const angle = Math.acos(THREE.MathUtils.clamp(this.alignNormal.y, -1, 1)) * this.alignPose;
    const horizontal = Math.hypot(this.alignNormal.x, this.alignNormal.z);
    if (horizontal < 1e-6) return this.skateCameraUpVector.copy(VERT_UP);
    const scale = Math.sin(angle) / horizontal;
    return this.skateCameraUpVector.set(this.alignNormal.x * scale, Math.cos(angle), this.alignNormal.z * scale);
  }
  ...
  get animationClipHint(): PlayerAnimationClipHint {
    ...
    if(this.state==='air'&&this.airFromSkate&&this.flipT>0&&(this.flipKind==='imposs'||this.flipKind==='varial'||this.flipKind==='varial-heel'||this.flipKind==='hardflip'||this.flipKind==='inward-heel'))return 'player.skate';
  ...
  resumeSuspendedLevel(
    ...
    this.runTrickUses = new Map(state.trickHistory ?? []);
    this.ttActive = false;
    this.comboRun = false;
  ...
  private settle(level: Level, facing?: THREE.Vector3): void {
    ...
    this.chargeTimer = 0;
    this.skateCharge = 0;
    this.freeSkate = level.skatepark;
  ...
  private grindTrickName(): string {
    return this.specialGrind?.label ?? GRIND_TRICKS[this.grindStyle].label;
  }
  ...
  private trickRepeat(label: string): number {
    return trickRepeatFactor((this.runTrickUses.get(label) ?? 0) + (this.comboUses.get(label) ?? 0));
  }
  ...
  private awardHeldScore(kind: 'grab'|'grind'|'manual'|'lip'|'wallride', seconds: number, rate: number): void {
    const held=this.heldTrickScores.get(kind);
    if(held){
      const pay=extendHeldTrick(held,rate,seconds);
      this.comboPoints+=pay;
      this.special.award(pay);
    }
    this.comboTimer=CONST.comboWindow;
  }

  private score(base: number, label?: string, held?: 'grab'|'grind'|'manual'|'lip'|'wallride'): { shown: string | undefined; pay: number } {
    const inTrick =
      this.landingScoring ||
      this.state === 'air' ||
      this.state === 'grind' ||
    ...
    if (inTrick && isTrick && label) {
      // Landed combos retain their uses for this run. Bailed attempts only
      // discard the pending uses, so a bail never refreshes landed history.
      repeat = this.trickRepeat(label);
      const uses = this.comboUses.get(label) ?? 0;
      this.comboUses.set(label, uses + 1);
      pay = Math.round(base * repeat);
    }
    ...
    if (inTrick && isTrick && held) {
      this.heldTrickScores.set(held, {raw:base,paid:pay,factor:repeat,
        power:this.uberTimer>0?CONST.uberScoreMult:1,seconds:0});
    }
    if (inTrick) {
      this.comboPoints += pay;
  ...
  private bankCombo(): void {
    for (const [name,count] of this.comboUses)
      this.runTrickUses.set(name,(this.runTrickUses.get(name)??0)+count);
  ...
  private resolveBailControlFrame(level: Level): void {
    ...
    if (laneDir || chaseMode) {
      const fx = laneDir?.x ?? (level.skatepark ? this.axisF.x : this.camDir.x);
      const fz = laneDir?.z ?? (level.skatepark ? this.axisF.z : this.camDir.z);
  ...
  private stepRide(dt: number, input: Input, level: Level): void {
    ...
    if (this.lipStallT > 0) {
      ...
      const lipRamp = Math.min(
        ...
        Math.max(0, this.balanceAge - TUNING.balanceGrace) * TUNING.balanceRamp * 2,
      );
      ...
      while (this.lipTickT >= 0.25) {
        this.lipTickT -= 0.25;
        this.awardHeldScore('lip',.25,CONST.ptsLipTick*4);
      }
    ...
    const rollingOut = this.freeSkate && (this.parkControls || Math.abs(this.speed) > 0.08 || (this.softSkateImpactT > 0 && !input.grabHeld)) && !this.stepOff;
    this.stepOff = false;
      ...
      if (this.freeSkate) {
        if (onPipe && steepGround && this.rideNormal.y > 0.25 && this.speed < 2 && this.pipeFlipCd <= 0) {
          ...
          if (Math.hypot(n.x, n.z) > 1e-3) {
            const hp = this.groundHit?.halfpipe;
            this.axisF.set(hp?.axis === 'x' ? this.axisF.x : Math.sign(n.x), 0,
              hp?.axis === 'x' ? Math.sign(n.z) : this.axisF.z);
    ...
    if (this.manualing !== 0) {
      if (this.manualCoyoteT > 0 || !this.grounded) {
        ...
        const ramp = Math.min(
          Math.max(0, TUNING.balanceRampMax - 1),
          Math.max(0, this.balanceAge - TUNING.balanceGrace) * TUNING.balanceRamp * 1.5,
        );
        ...
        control *= this.safeGain(this.balanceEntryAge, control, runSign);
        const settle = this.balanceEntryEase(TUNING.manualCalm);
        this.stepBalanceCore(dt, runSign, instability * settle, control, ramp, settle);
        if (this.uberTimer > 0 || this.balanceBoostT > 0) {
          this.balance = 0;
          this.balanceVel = 0;
        }
        ...
        while (this.manualTickT >= 0.25) {
          ...
          this.awardHeldScore('manual',.25,CONST.ptsManualTick*4);
        }
      }
    }
    ...
    if (parkLipExit) {
      ...
      (meshCrest || this.pos.y > hpNow!.lipY - 1.2) // and up near the lip
    ) {
      ...
      if (hpNow && this.lipCoolT <= 0 && this.rawInput.grindHeld && this.lipHeadOn(hpNow)) {
        this.enterLipStall(hpNow);
        return;
      }
      this.state = 'air';
      this.grounded = false;
      this.groundHit = meshCrest ? this.groundHit : hit;
      this.airFromSkate = true;
      this.airGrav = 'board';
      this.pipeHang = true; // climb-hold must not read as a trick-spin (no phantom bail)
      this.hangPipe = hpNow ?? null; // remember which pipe launched this hang (spine transfers)
      this.grabSpinAngle = 0;
  ...
  private stepParkGroundMotor(dt: number, input: Input): boolean {
    ...
    // No-input rollout borrows only the platform friction curve. The park's
    // steering, brake, slope projection and air rules above/below stay intact.
  ...
  private stepAir(
    ...
    const transitionCatch = !this.wallriding && !pipeCatch ? this.sweepTransitionContact(level) : null;
    if (transitionCatch) hit = transitionCatch;
    if (pipeCatch) {
    ...
    let landNow =
      hit !== null &&
      (pipeCatch !== null || transitionCatch !== null || // swept contacts already resolved the contact exactly
        (this.vVel <= 0 &&
    ...
    if (landNow && hit) {
      ...
      // This fixed step includes the final catch frame. Settle it before the
      // landing check rather than throwing away a completed trick by order.
      if(this.flipT>0 && this.flipT<=dt+1e-7)this.completeDeckTrick();
      ...
      const funny = (spun && offLine > sketchNet) || lateSpecialFlip || (this.parkControls && flipLate);
      ...
      if (this.grabPhase !== 'none' || funny) {
        ...
        if (!completedSpecialGrab && halves >= (wasPipeHang && !this.parkControls ? CONST.vertSpinMin : 1)) {
          ...
          if (this.airGrabShown) {
            ...
            let fold = Math.round(spinBase * this.trickRepeat(spinName));
            this.comboUses.set(spinName, (this.comboUses.get(spinName) ?? 0) + 1);
  ...
  private enterVertAir(launch: boolean): void {
    ...
    if (this.pipeRideT > 0 || this.groundHit?.vert === true) {
      this.pipeHang = true;
  ...
  private tryManual(type: 1 | -1): void {
    ...
    if (this.state === 'air') {
      if (this.airFromSkate) {
        ...
        this.manualArmT = TUNING.manualArmWindow;
      }
      return;
    }
  ...
  private stepBalanceCore(
    ...
    settle = 1, // quiet catch noise too; corrective input stays available
  ): void {
    ...
    if (TUNING.balanceNoise > 0) {
      ...
      if (quiet > 0) {
        ...
        force += n * TUNING.balanceNoise * (1 + ramp) * quiet * settle;
      }
    }
  ...
  private balanceEntryEase(seconds: number): number {
    return seconds > 0 ? THREE.MathUtils.smoothstep(this.balanceEntryAge, 0, seconds) : 1;
  }

  // Catch directions also select tricks. Ease their OUTWARD influence while
  // they are released; inward correction always has full authority. This is
  // not an end-of-meter grace or a reset of the carried needle.
  private safeGain(t: number, control: number, runSign: number): number {
    if (TUNING.balanceSafePeriod <= 0) return 1;
    if (Math.sign(control) !== runSign) return 1;
    return THREE.MathUtils.smoothstep(t, 0, TUNING.balanceSafePeriod);
  }
  ...
  private enterLipStall(hp: Halfpipe): void {
    ...
    this.rideNormal.set(0, 1, 0);
    // Either side input requests a tip stall. The physical board end that
    // arrived at the deck determines nose/tail; never turn it to fit a name.
    const rIn = this.rawInput;
    const noseYaw = this.visualYaw + this.deckYawOffset;
    const noseOut = this.lipSide * (hp.axis === 'z' ? -Math.sin(noseYaw) : -Math.cos(noseYaw));
    this.lipStyle =
      rIn.moveY < -0.4
        ? 'rock'
        : Math.abs(rIn.moveX) > 0.4 ? noseOut >= 0 ? 'nose' : 'tail' : 'axle';
  ...
  private stepGrind(dt: number, input: Input, level: Level): void {
    ...
    if (input.grindPressed && this.lipStallT <= 0) {
      ...
      if (nextSpecial || this.grindStyle !== prev || prevSpecial !== null) {
        ...
        } else this.special.consumeInput();
        this.score(
          nextSpecial?.points ?? GRIND_TRICKS[this.grindStyle].points,
          this.grindTrickName(), 'grind',
        );
        this.grindStyleT = 0; this.grindTickT = 0;
    ...
    // Read live tuning against the current catch, not the combo's age.
    const calm = this.balanceEntryEase(TUNING.grindCalm);
    ...
    while (this.grindTickT >= 0.25) {
      ...
      const travel=Math.min(1,Math.abs(this.grindVel)/4);
      const rate=this.specialGrind?500:GRIND_TRICKS[this.grindStyle].rate;
      this.awardHeldScore('grind',.25,rate*travel);
    }
  ...
  private tryGrind(pressed: boolean, level: Level): boolean {
    ...
    // Do not flash a grind/needle for a catch that will run off its endpoint
    // within two physics ticks. Inward catches at the same end remain valid.
    const vertVelocity = this.parkControls && this.vertAir && !this.grounded;
  ...
  private enterGrind(rail: Rail, sample: RailSample, level?: Level): void {
    ...
    this.grindTickT = 0; this.grindStyleT = 0;
    ...
    this.score(
      entrySpecial?.points ?? GRIND_TRICKS[this.grindStyle].points,
      this.grindTrickName(), 'grind',
    );
    ...
    this.beginBalance((this.simRand() < 0.5 ? -1 : 1) * TUNING.balanceEntryLean);
  ...
  private tryStartDeckTrick(intent: boolean, x=this.rawInput.moveX, y=this.rawInput.moveY): boolean {
    if (
      !intent ||
      this.state !== 'air' ||
      !this.airFromSkate ||
    ...
    this.specialFlip = null;
    this.flipKind = deckTrickFromInput(x,y);
    const trick=deckTrickInfo(this.flipKind);
    this.flipName = trick.label;
    this.flipDuration = this.flipT = trick.duration;
  ...
  private tryStartSpecialFlip(): boolean {
    ...
    this.grabSpinAngle=Math.round(this.grabSpinAngle/Math.PI)*Math.PI;
  ...
  private updateSpin(dt: number, input: Input): void {
    ...
    if(input.spinPressed && (boardAir || bufferOllieDeckTrick)) {
      ...
      if(releasingGrab){
        ...
        this.grabBlockedUntilRelease=true;
      }
      if(bufferOllieDeckTrick)this.ollieDeckTrickBufferT=OLLIE_DECK_TRICK_CHORD;
    }
    let specialFlipStarted=false;
    if(boardAir && this.queuedFlip){
      const queued=this.queuedFlip;
      this.pendingSpecialFlip=this.specialReady?queued.special:null;
      specialFlipStarted=this.tryStartSpecialFlip();
    ...
    if (
      input.spinPressed &&
      !specialFlipStarted &&
      !(this.parkControls && this.freeSkate) &&
      !this.spinning &&
      this.spinCd <= 0 &&
      canSpin
    ) {
      ...
      if (this.state === 'air' && !this.airFromSkate && this.vVel < 7) {
        // Tiny Crash-style stall. Never boosts an already-rising jump.
        this.vVel = Math.min(this.vVel + TUNING.spinAirCorrection, 7);
      }
      // Outside skate parks the hybrid game retains its ordinary attack.
      // Deck tricks above have independent input timing and no flight impulse.
    ...
    if (!boardAir && !this.charging) {
      this.ollieDeckTrickBufferT=0;this.queuedFlip=null;
    }
    if (this.spinTimer > 0) {
      this.spinTimer -= dt;
    ...
    if (this.flipT > 0) {
      ...
      if (this.flipT <= 0) {
        ...
        if (
          ...
          this.completeDeckTrick();
          // Preserve the hybrid attack cadence outside parks. Deck input
          // itself is independent of this legacy attack cooldown.
  ...
  private startGrab(kind:GrabTrickKind):void {
    ...
    this.special.consumeInput();
    this.grabPhase='enter';this.grabT=0;this.grabTickT=0;
    const scored=this.score(trick.points,trick.label,'grab');
    this.airGrabShown=scored.shown??null;
    sfx.play('woosh2',.4);
  }
  ...
  private updateGrab(dt: number, input: Input): void {
    ...
    if(!input.grabHeld){this.grabBlockedUntilRelease=false;this.queuedGrab=null;}
    if(this.queuedGrab){this.queuedGrab.time-=dt;if(this.queuedGrab.time<=0)this.queuedGrab=null;}
    const boardAir=this.state==='air'&&this.airFromSkate&&!this.isBailing&&!this.slamActive&&!this.wallriding;
    if(input.grabPressed&&input.grabHeld&&boardAir){
      this.queuedGrab={kind:grabTrickFromInput(this.rawInput.moveX,this.rawInput.moveY),time:.18,special:this.pendingSpecialGrab};
    }
    if(this.queuedGrab&&boardAir&&this.flipT<=0&&!this.specialGrab&&
        (this.grabPhase==='none'||this.grabPhase==='exit')&&!this.grabBlockedUntilRelease){
      const queued=this.queuedGrab;this.pendingSpecialGrab=this.specialReady?queued.special:null;
      if(!this.tryStartSpecialGrab())this.startGrab(queued.kind);
      this.queuedGrab=null;
    }
    if(!boardAir)this.queuedGrab=null;
    if (this.specialGrab) {
    ...
    if (this.state === 'air') {
      // Park grabs start from a fresh buffered Circle press. Once started,
      // they retain the entry variant when the stick re-centers or rotates.
      ...
      if (
        ...
        (grabActive || (!this.parkControls && (grabDir || input.grabPressed)))
      ) {
        // Reach into the pose over grabTransition, then hold it.
        if (this.grabPhase === 'none' || this.grabPhase === 'exit') {
          this.startGrab(grabTrickFromInput(this.rawInput.moveX,this.rawInput.moveY));
        ...
        // The entry direction owns the grab; steering/spinning cannot
        // rename it, refund history or move the grabbing hand mid-trick.
        // THPS accrual: held grabs are worth more
        this.grabTickT += dt;
        while (this.grabTickT >= 0.25) {
          this.grabTickT -= 0.25;
          this.awardHeldScore('grab',.25,grabTrickInfo(this.grabKind).rate);
        }
      } else {
        ...
        if ((this.vertAir || streetSpin) && !this.specialFlip && !this.slamActive && hsp !== 0) {
          if (!this.parkControls || this.parkSpinHold >= SKATE_PARK.spinDelay)
  ...
  private tryWallride(w: THREE.Box3, level: Level): boolean {
    ...
    this.rollOffT = 0;
    this.score(CONST.ptsWallride, 'Wallride', 'wallride'); // timed trick: shows the combo plate straight away, then ticks up
  ...
  private stepWallride(dt: number, input: Input, level: Level): void {
    ...
    while (this.wallTickT >= 0.25) {
      this.wallTickT -= 0.25;
      this.awardHeldScore('wallride',.25,CONST.ptsWallrideTick*4);
    }
  ...
  /** Sweep the completed flight segment against the actual transition
   * triangles. Catches steep angled landings before a down-ray can see the
   * foundation below; backside/under-shell approaches are never landings. */
  private sweepTransitionContact(level: Level): GroundHit | null {
    VERT_RAY_O.copy(this.prevPos).addScaledVector(VERT_UP, 0.025);
    VERT_RAY_D.subVectors(this.pos, VERT_RAY_O);
    const distance = VERT_RAY_D.length();
   ...
   */
  private applyBackflipPose(progress:number):void {
    const pose=sampleBackflip(progress);
    const side=-this.stance*Math.PI/2*this.sidePose;
    _trickFrameQ.setFromAxisAngle(VERT_UP,side);
    _trickRotationQ.setFromAxisAngle(_trickRight,pose.rotation);
    _trickRotationQ.premultiply(_trickFrameQ).multiply(_trickFrameQ.invert());
    // Rotate the attached board and rider around their posed hips.
    if (this.legs) {
    ...
    this.bodyGroup.quaternion.multiply(_trickRotationQ);
  }
  ...
  private syncVisual(input: Input, dt: number): void {
    ...
    if (this.boardG) {
      ...
      this.boardG.rotation.y = -this.stance * (Math.PI / 2) * this.sidePose + this.deckYawOffset;
    ...
    if(ollieMotion){
      const lengths=this.specialFlip
        ...
        :this.flipT>0&&(this.flipKind==='kick'||this.flipKind==='heel'||this.flipKind==='shove'||this.flipKind==='varial'||this.flipKind==='varial-heel'||this.flipKind==='hardflip'||this.flipKind==='inward-heel')?skateFootFlipElasticity(sampleFootFlip(this.flipKind,1-this.flipT/this.flipDuration),ollieMotion.deformations,this.stance):this.flipT>0&&this.flipKind==='imposs'?skateImpossibleElasticity(sampleImpossible(1-this.flipT/this.flipDuration),ollieMotion.deformations,this.stance):ollieMotion.deformations;
    ...
    const deckTrickPose=this.flipT>0&&this.state==='air'&&!this.specialFlip
      ?sampleDeckTrick(this.flipKind,1-this.flipT/this.flipDuration):null;
    if(deckTrickPose){
      if(this.kneeL)this.kneeL.rotation.x+=.45*deckTrickPose.tuck;
      if(this.kneeR)this.kneeR.rotation.x+=.45*deckTrickPose.tuck;
      const front=this.stance>0?this.legL:this.legR;
    ...
    if (this.boardG && this.riderG) {
      ...
      if(copingPipe && this.pos.y>copingPipe.lipY-1.8){
        ...
        const normal=new THREE.Vector3(copingPipe.axis==='z'?side:0,0,copingPipe.axis==='x'?side:0);
        const center=this.pos.clone();center.y=copingPipe.lipY+.05;
        if(copingPipe.axis==='z')center.x=copingPipe.cross+side*copingPipe.lipX;
        else center.z=copingPipe.cross+side*copingPipe.lipX;
        coping={center,normal};
      }
    ...
    if(deckTrickPose&&this.boardG&&!skateContactOwned){
      ...
      this.boardG.position.add(_trickOffset);
      this.boardG.rotateZ(deckTrickPose.roll);
      this.boardG.rotateY(deckTrickPose.yaw);
