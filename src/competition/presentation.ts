...
export class CompetitionPresentation {
  ...
  private previous = {accept:false,back:false,up:false,down:false};
  private selected = 0;
  ...
  constructor(private action: (action: CompetitionAction) => void, readonly hooks: JudgePresentationHooks = {}) {
    ...
    this.element.addEventListener('click', e => {
      ...
      if (button && !button.disabled) this.action(button.dataset.action as CompetitionAction);
    });
    window.addEventListener('keydown', e => {
      if (!this.modalActive || e.repeat || document.body.classList.contains('game-shell-modal')) return;
      const buttons = this.buttons();
      if (e.code === 'ArrowDown' || e.code === 'ArrowRight') {e.preventDefault();this.select(1);}
      else if (e.code === 'ArrowUp' || e.code === 'ArrowLeft') {e.preventDefault();this.select(-1);}
      else if (e.code === 'Enter' || e.code === 'Space') {e.preventDefault();buttons[this.selected]?.click();}
      else if (e.code === 'Tab' && buttons.length) {e.preventDefault();this.select(e.shiftKey?-1:1);}
    });
  }
  ...
  private select(delta: number): void {
    ...
    this.selected=(this.selected+delta+buttons.length)%buttons.length;buttons[this.selected].focus();
  }
  updateInput(): void {
    if (!this.event) return;
    const pad=inputPrompts.gamepad;
    ...
    if(!this.seedInput&&this.modalActive&&!document.body.classList.contains('game-shell-modal')){
      if(next.up&&!this.previous.up)this.select(-1);
      if(next.down&&!this.previous.down)this.select(1);
      if(next.accept&&!this.previous.accept)this.buttons()[this.selected]?.click();
    }
  ...
  render(event: JungleCupEvent | null, suppressed = false): void {
    ...
    if (this.event !== event) { this.key=''; this.revealed=0; this.revealRun=0; this.seedInput=true; }
    ...
    if(event.phase!=='running'&&event.phase!=='countdown') {
      ...
      target?.focus({preventScroll:true});this.selected=Math.max(0,this.buttons().indexOf(target!));
    }
  }
