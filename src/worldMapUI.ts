...
export class WorldMapUI {
  ...
  private render(immediate = false): void {
    ...
    if (trialUnlocked) {
      ...
      records.textContent = `Time trial. Your best times: ${[0, 1, 2].map(i => `${i + 1}: ${mapTrialTime(times[i])}`).join(", ")}. Medals: ${TIME_MEDALS.map((tier, i) => `${tier}: ${medal && i >= TIME_MEDALS.indexOf(medal) ? 'earned' : mapTrialTime(targets[tier])}`).join(", ")}.`;
