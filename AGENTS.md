...
4. Run focused checks relevant to the change (`npm run check:levels` after editing level data) and `npm run build` before handoff. Build only type-checks and bundles. Run the full suite (`npm run check:all`) only when the user explicitly asks; normal publishing must not trigger it. Smoke-test the affected level in a real browser and check the console for errors.
