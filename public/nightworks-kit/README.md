...
The complete course is source-owned in `src/levels/nightworks.ts`. A single fixed-heading camera spine keeps controls stable across its switchbacks; the player supplies course turns without side-scroll input remapping. Existing travel direction, sine timings, checkpoints and summit gate remain; two touching pairs of rest slabs are joined into single islands to avoid seams. All seven models also appear in the editor asset catalog. The `travelPhase` rope field preserves the third ferry's independent anchor phase through editing and export.
