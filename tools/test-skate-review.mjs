...
await withSkateRuntime(async ({player:p,server,THREE})=>{
  ...
  for(const entry of SKATE_REVIEW_ENTRIES){
    ...
    if(entry.id==='grab:indy'){
      assert.equal(clip.metadata.transitionCapture,'native Player.step inputs');assert.equal(clip.metadata.captureFps,60);
      assert.ok(trace.some(s=>s.grabPhase==='held')&&trace.at(-1).grounded&&trace.at(-1).grabWeight===0,'Indy must enter, grab, release and land');
    }
    ...
    if(['flip:kick','flip:heel','flip:shove','flip:imposs','flip:varial','flip:varial-heel','flip:hardflip','flip:inward-heel'].includes(entry.id)){
      assert.equal(clip.metadata.transitionCapture,'native Player.step inputs');
      assert.equal(clip.metadata.captureFps,['flip:imposs','flip:varial','flip:varial-heel','flip:hardflip','flip:inward-heel'].includes(entry.id)?240:60);
      assert.ok(trace.some(s=>s.flipping)&&trace.at(-1).grounded,`${entry.number} must show a real pop, flick and landing`);
    }
    ...
    if(entry.id==='grind:under'){
      ...
      assert.equal(at(.2).underFlag,false);assert.equal(at(.95).underFlag,true);
      assert.equal(at(2.1).underFlag,true);
      assert.equal(at(2.4).state,'air');assert.equal(at(3.8).grounded,true);
      const rootY=t=>a.sampleComposedClip(clip,t,motion).joints.skateBody.position[1];
      assert.ok(rootY(.2)-rootY(.95)>1.6,'under-rail transition has no visible vertical travel');
      assert.ok(rootY(3.8)<.25,'the drop never reaches the floor');
    }
    for(let f=0;f<45;f++){
