...
function start(name:string,p:number[],h:number[],input:(f:number,p:any)=>any,frames=720,speed=23){g.competitionAction('retry');route={name,p,h,input,frames,speed,firstLanding:true};frame=0;events=[];freeze=false;placed=false;firstAir=false;firstLanding=false;lastVert=false;previousY=0;}
const push=(f:number)=>({moveY:1,jumpHeld:true,jumpPressed:f===0});
