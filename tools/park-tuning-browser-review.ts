...
p.step=(dt:number,input:any,level:any)=>{
 if(freeze)return;const command=neutral();command.moveY=mode==='cruise'||mode==='charge'?1:0;command.grabHeld=mode==='brake';command.jumpHeld=mode==='charge'||mode==='vert'||(mode==='ollie'&&frame<Math.ceil(TUNING.parkOllieChargeTime/dt)+1);command.jumpPressed=frame===0&&command.jumpHeld;command.jumpReleased=mode==='ollie'&&frame===Math.ceil(TUNING.parkOllieChargeTime/dt)+1;
