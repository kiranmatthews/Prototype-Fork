...
try {
  ...
  try {
    ...
    assert.equal(parentLevel.bonusPlatformAt(new THREE.Vector3(trigger.x,trigger.y-1,trigger.z)),false,'running underneath the raised deck must not trigger entry');
    const standing={enabled:true,grounded:true,jump:false,rising:false};
    assert.equal(parentLevel.consumeBonusLanding(trigger,standing),false,'standing or walking onto the deck is not a jump');
    assert.equal(parentLevel.consumeBonusLanding(trigger,{...standing,grounded:false,rising:true}),false);
    assert.equal(parentLevel.consumeBonusLanding(trigger,standing),false,'falling onto the deck must not arm entry');
    assert.equal(parentLevel.consumeBonusLanding(trigger,{...standing,grounded:false,jump:true,rising:true}),false);
    assert.equal(parentLevel.consumeBonusLanding(trigger,standing),true,'a deliberate jump landing on top should enter');
    assert.equal(parentLevel.consumeBonusLanding(trigger,standing),false,'one jump must only enter once');
    parentLevel.consumeBonusLanding(trigger,{...standing,grounded:false,jump:true,rising:true});
    parentLevel.consumeBonusLanding(trigger.clone().add(new THREE.Vector3(5,0,0)),standing);
    assert.equal(parentLevel.consumeBonusLanding(trigger,standing),false,'an earlier jump landing elsewhere cannot arm a later walk-on');
    const previous=new THREE.Vector3(trigger.x+3,trigger.y-1.05,trigger.z),contact=previous.clone().add(new THREE.Vector3(-2,0,0)),normal=new THREE.Vector3();
    assert.equal(parentLevel.resolveBonusPlatformContact(previous,contact,new THREE.Vector3(.3,1,.3),normal),true,'stone sides should block a running approach');
    assert.ok(contact.x-trigger.x>=1.9);
    assert.equal(parentLevel.resolveBonusPlatformContact(previous,trigger.clone(),new THREE.Vector3(.3,1,.3),normal),false,'jumped feet above the top must clear the sides');
