...
await withSkateRuntime(async({player:p,server,THREE,level,step})=>{
  ...
  for(const stance of (process.env.MUTE_DEBUG?[Number(process.env.MUTE_DEBUG)]:[-1,1]))for(const deck of (process.env.MUTE_REVERSED?[Math.PI]:process.env.MUTE_DEBUG?[0]:[0,Math.PI]))for(const [position,heading] of (process.env.MUTE_DEBUG?walls.slice(0,1):walls)){
    ...
    for(let f=0;f<150;f++){
      ...
      step(input);frames++;p.group.updateMatrixWorld(true);assert.equal(p.isBailing,false,'Mute bailed');
      ...
      if(contact){
        started=true;assert.equal(contact.kind,'mute');assert.equal(p.stance,stance);
        ...
        if(p.grabPose>.95){
          ...
          for(const limbSide of ['left','right']){
            for(const part of ['upper','lower'])assert.ok(p.playerAnimationBridge.deformationValue(`deform.leg.${part}.${limbSide}.length`)>.90,'Mute must retain leg length');
          }
  ...
  console.log('PASS native Mute seated hips, upper-body fold, clear limbs and toe-edge grip, planted feet, clearance and clean release/landing.');
});
