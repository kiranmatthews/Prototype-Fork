...
await withSkateRuntime(async({player:p,server,THREE,level,step})=>{
  ...
  for(const stance of (process.env.METHOD_DEBUG?[Number(process.env.METHOD_DEBUG)]:[-1,1]))for(const deck of (process.env.METHOD_DEBUG?[0]:[0,Math.PI]))for(const [position,heading] of (process.env.METHOD_DEBUG?walls.slice(0,1):walls)){
    ...
    for(let f=0;f<150;f++){
      ...
      step(input);frames++;p.group.updateMatrixWorld(true);assert.equal(p.isBailing,false,'Method bailed');
      ...
      if(contact){
        started=true;assert.equal(contact.kind,'method');assert.equal(p.stance,stance);
        ...
        if(p.grabPose>.95){
          ...
          for(const limbSide of ['left','right']){
            for(const part of ['upper','lower'])assert.ok(p.playerAnimationBridge.deformationValue(`deform.leg.${part}.${limbSide}.length`)>1.18,'Method leg segments must lengthen');
          }
          ...
          if(hip<.55||Math.max(...knees)>1.70||waist<.65||waist>2.10)fail({kind:'posture',stance,deck,f,hip,knees,waist});
        }
  ...
  console.log('PASS native Method extended legs, head-clear arms, inward palm and heel-edge grip, planted feet, clearance and clean release/landing.');
});
