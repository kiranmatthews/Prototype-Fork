...
import { SKATE_REVERT_DURATION } from '../src/skateBodyMotion';
...
button('Revert',()=>start('revert'));button('Flat Kickflip',()=>{flip.value='Kickflip';start('flat-flip');});button('Flat Heelflip',()=>{flip.value='Heelflip';start('flat-flip');});button('Flat Pop Shove-It',()=>{flip.value='Pop Shove-It';start('flat-flip');});button('Flat Impossible',()=>{flip.value='Impossible';start('flat-flip');});button('Flat Varial Kickflip',()=>{flip.value='Varial Kickflip';start('flat-flip');});button('Flat Varial Heelflip',()=>{flip.value='Varial Heelflip';start('flat-flip');});button('Flat Hardflip',()=>{flip.value='Hardflip';start('flat-flip');});button('Flat Inward Heelflip',()=>{flip.value='Inward Heelflip';start('flat-flip');});
...
g.player.step=(dt:number,input:any,level:any)=>{
  ...
  if(hadAir&&p.grounded&&scenario!=='revert'){freeze=true;logs.push(`landed ${p.comboPoints} × ${p.comboMult}`);}
  if(scenario==='revert'&&reverted&&p.revertPoseT<=0){freeze=true;logs.push(`revert complete in stance ${p.stance}`);}
