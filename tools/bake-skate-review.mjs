...
await withSkateRuntime(async ({player:p,Level,server,THREE})=>{
  ...
  for(const entry of SKATE_REVIEW_ENTRIES){
    ...
    const nativeRevert=id==='Revert',nativeGrab=category==='grab'&&(id==='indy'||id==='melon'||id==='nose'||id==='tail'||id==='method'||id==='mute'||id==='stalefish');
    // Downward grab inputs select Tailgrab/Stalefish in parks; campaign uses slam.
    level.skatepark=nativeGrab&&(id==='tail'||id==='stalefish');
      ...
      if(native&&time>=0){
        ...
        if(nativeFootFlip){
          ...
          if(p.state==='air'){
            ...
            if(indyAirFrames===3){command.moveX=(id==='melon'||id==='method'||id==='stalefish')?-1:(id==='nose'||id==='tail')?0:1;command.moveY=(id==='nose'||id==='method'||id==='mute')?1:(id==='tail'||id==='stalefish')?-1:0;command.grabPressed=true;}
          }
