...
await withSkateRuntime(async ({player:p,Level,server,THREE})=>{
  ...
  for(const entry of SKATE_REVIEW_ENTRIES){
    ...
    const stateTrace=[];
    let lipEnteredAt=null,lipExitSent=false,landedAt=null;
    ...
    function pose(time){
      if(native&&time>=0){
        ...
        if(nativeLip&&landedAt!==null&&time>landedAt+.25){p.runTime+=dt;p.syncVisual(makeInput(),dt);}
      ...
      p.grabSpinAngle=id==='the-900'?Math.PI*5*smooth:id==='kickflip-mctwist'&&specialProgress===1?Math.PI*3:0;
      ...
      p.boardOllieAir=id==='Ollie'&&airborne;p.wallriding=id==='Wallride';p.wallridePose=p.wallriding?1:0;p.wallNormal.set(1,0,0);
    ...
    for(let f=0;f<=Math.round(entry.duration*60);f++){
      ...
      const offset=new THREE.Vector3(native?p.pos.x:0,p.pos.y,nativeLip?p.pos.z:0).applyQuaternion(canonical.clone().invert());
    ...
    if(native){
      ...
      if(nativeLip)clip.metadata.reviewPipe=true;
    }
