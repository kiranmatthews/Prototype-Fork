...
await withSkateRuntime(async ({player:p,Level,server,THREE})=>{
  ...
  for(const entry of SKATE_REVIEW_ENTRIES){
    ...
    const nativeRevert=id==='Revert',nativeGrab=category==='grab'&&(id==='indy'||id==='melon'||id==='nose'||id==='tail'||id==='method'||id==='mute'||id==='stalefish'||id==='japan');
    // Downward grab inputs select Tailgrab/Stalefish/Japan in parks; campaign uses slam.
    level.skatepark=nativeGrab&&(id==='tail'||id==='stalefish'||id==='japan');
      ...
      if(native&&time>=0){
        ...
        if(nativeFootFlip){
          ...
          if(p.state==='air'){
            ...
            if(indyAirFrames===3){command.moveX=(id==='melon'||id==='method'||id==='stalefish')?-1:(id==='nose'||id==='tail')?0:1;command.moveY=(id==='nose'||id==='method'||id==='mute')?1:(id==='tail'||id==='stalefish'||id==='japan')?-1:0;command.grabPressed=true;}
          }
        ...
        if((nativeLip||nativeBackflip||nativeFootFlip||nativeGrab)&&landedAt!==null&&time>landedAt+.25){p.runTime+=dt;p.syncVisual(makeInput(),dt);}
        ...
        if(nativeGrab&&indyAirFrames>0&&p.grounded&&landedAt===null)landedAt=time;
        ...
        stateTrace.push({time,state:p.state,under:p.underK,underFlag:p.railUnder,lip:p.lipStallT>0,wall:p.wallriding,manual:p.manualing,grounded:p.grounded,y:p.pos.y,
          ...
          ...(nativeGrab?{grabPhase:p.grabPhase,grabWeight:p.grabPose,stance:p.stance,deckYaw:p.deckYawOffset}:{})});
        if(nativeGrind&&f===(id==='darkslide'?2:0)){
    ...
    if(native){
      ...
      if(id==='under'||nativeFootFlip||nativeRevert||nativeGrab)clip.metadata.captureFps=tickFps;
