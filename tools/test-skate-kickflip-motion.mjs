...
await withSkateRuntime(async({player:p,server,THREE,Level,Player})=>{
  ...
  const {sampleKickflip}=await server.ssrLoadModule('/src/skateTricks.ts');
  ...
  for(const park of [false,true])for(const stance of [-1,1])for(const reversed of [false,true])for(const delay of [1,9]){
    ...
    runtime.restart();let air=0,started=false,landAge=0,previous=null,coast=[],catchFrames=[],flicked=false;
    for(let f=0;f<220;f++){
      if(p.state==='air')air++;
      const input=makeInput({jumpHeld:f>=30&&f<60,jumpPressed:f===30,jumpReleased:f===60,spinPressed:air===delay,spinHeld:air===delay});
      p.step(1/60,input,level);control.step(1/60,input,level);level.update(1/60);frames++;
      assert.ok(p.pos.distanceTo(control.pos)<1e-9,'Kickflip presentation changed controller motion');
      ...
      const m=p.boardG.userData.skateKickflip;
      if(m){
        started=true;const t=1-p.flipT/p.flipDuration;
        const error=centre.clone().sub(p.group.getWorldPosition(v())).sub(v(0,centreY,0)).length();maxArcError=Math.max(maxArcError,error);
        ...
        if(t>.3&&t<.65){
          ...
          const right=v(0,1,0).cross(nose).normalize(),up=nose.clone().cross(right).normalize();
        ...
        if(previous){const delta=centre.clone().sub(previous.centre).sub(p.pos.clone().sub(previous.pos));maxStep=Math.max(maxStep,delta.length());}
        previous={centre, pos:p.pos.clone()};
        if(error>.004&&failures.length<8)failures.push({kind:'arc',park,stance,reversed,delay,t,error});
        if(contact.footError>.008&&failures.length<8)failures.push({kind:'foot',park,stance,reversed,delay,t,error:contact.footError});
      }
      if(m)for(const mesh of shoes){
        ...
        for(let i=0;i<mesh.geometry.attributes.position.count;i++){
          ...
          if(gap<-.004&&failures.length<8)failures.push({kind:'shoe',park,stance,reversed,delay,t:1-p.flipT/p.flipDuration,mesh:mesh.name,gap});
        }
      }
      if(started){
        ...
        for(let i=0;i<shorts.geometry.attributes.position.count;i++){
          ...
          if(gap<.02&&failures.length<8)failures.push({kind:'shorts',park,stance,reversed,delay,f,gap});
        }
    ...
    for(let i=1;i<coast.length;i++){
      ...
      angularSpeed??=speed;assert.ok(Math.abs(speed-angularSpeed)<.01,'visible board does not keep its angular momentum after the kick');
    }
  ...
  assert.equal(sampleKickflip(1).turn,1);assert.equal(sampleKickflip(.10).turn,0);
  ...
  console.log('PASS native Kickflip impulse, independent deck-centre flight, downward foot catches, clearance and unchanged movement in both stances/deck orientations.');
});
