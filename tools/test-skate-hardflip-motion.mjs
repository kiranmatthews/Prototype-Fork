...
await withSkateRuntime(async({player:p,server,THREE,Level,Player})=>{
  ...
  const level=new Level(new THREE.Scene(),{id:'hardflip-test',name:'Hardflip test',data:{v:1,name:'Hardflip test',spawn:[0,.02,0],killY:-30,
  ...
  for(const kind of ['hardflip'])for(const park of (process.env.HARDFLIP_DEBUG?[false]:[false,true]))for(const stance of (process.env.HARDFLIP_DEBUG?[-1]:[-1,1]))for(const reversed of (process.env.HARDFLIP_DEBUG?[false]:[false,true]))for(const delay of (process.env.HARDFLIP_DEBUG?[1]:[1,9])){
    cases++;level.skatepark=park;const sign=-1,parity=reversed?-1:1;
    for(const rider of [p,control]){
      rider.enterLevel('hardflip-test');rider.respawn(level,true);rider.parkControls=park;rider.freeSkate=true;rider.speed=8;rider.parkVelocity.set(0,0,-8);
    ...
    runtime.restart();let air=0,started=false,landAge=0,clearanceKick=false,steep=false,tailLoad=false,balanced=false,initialNose=null,initialBody=null,lastYaw=null,lastRoll=null,yawTotal=0,rollTotal=0;const catches=[];
    for(let f=0;f<230;f++){
      if(p.state==='air')air++;
      ...
      assert.ok(p.pos.distanceTo(control.pos)<1e-9,'hardflip animation changed movement');assert.equal(p.isBailing,false);
      ...
      if(m){
        ...
        const noseRise=v(0,0,parity).transformDirection(p.boardG.matrixWorld).dot(v(0,1,0).applyQuaternion(frame));maxNoseRise=Math.max(maxNoseRise,noseRise);if(noseRise>.85)steep=true;
        if(t>.18){
    ...
    if(!(started&&clearanceKick&&steep&&tailLoad&&balanced&&landAge>20))fail({kind:'phases',trick:kind,park,stance,reversed,delay,started,clearanceKick,steep,tailLoad,balanced,landAge});
    ...
    assert.equal(p.comboPoints,300);assert.equal(p.comboMult,1);
  }
  ...
  console.log('PASS native frontside scoop/toe flick, steep hardflip passage, half-shove/full roll, asymmetric arm balance, independent flight, downward catch, mesh clearance and unchanged movement.');
});
