...
await withSkateRuntime(async({player:p,server,THREE,level,step})=>{
  ...
  const fail=e=>{if(errors.length<12)errors.push(e);};
  for(const stance of (process.env.INDY_DEBUG?[Number(process.env.INDY_DEBUG)]:[-1,1]))for(const deck of (process.env.INDY_DEBUG?[0]:[0,Math.PI]))for(const [position,heading] of (process.env.INDY_DEBUG?walls.slice(0,1):walls)){
    ...
    for(let f=0;f<150;f++){
      ...
      step(input);frames++;p.group.updateMatrixWorld(true);assert.equal(p.isBailing,false,'Indy bailed');
      ...
      if(contact){
        started=true;assert.equal(contact.kind,'indy');assert.equal(p.stance,stance);
        ...
        if(p.grabPose>.95){
          ...
          const palm=p.boardG.worldToLocal(at(contact.hand));assert.ok(palm.x*stance*(deck? -1:1)>s.deckHalfWidth*k*.9&&Math.abs(palm.z)<.3);
          ...
          if(hip<.75||Math.max(...knees)>1.20||waist<.80||waist>1.90)fail({kind:'posture',stance,deck,f,hip,knees,waist});
        }
  ...
  console.log('PASS native Indy waist reach, upright legs, trailing toe-edge grip, planted feet, clearance and clean release/landing.');
});
