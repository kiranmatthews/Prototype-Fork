...
const provenance=JSON.parse(await fs.readFile(new URL('public/fonts/roo-font-v5-provenance.json',root),'utf8'));
...
try{
 ...
 const zip=page.waitForEvent('download');await page.locator('#atlas').click();await(await zip).saveAs(out+'/roo-image-font-v5.zip');
 const{unzipSync}=await import('../../node_modules/three/examples/jsm/libs/fflate.module.js');
 const zipFiles=unzipSync(await fs.readFile(out+'/roo-image-font-v5.zip'));
 assert.equal(Object.keys(zipFiles).length,10);
 assert.equal(JSON.parse(new TextDecoder().decode(zipFiles['roo-font-v5-provenance.json'])).glyphCount,51);report.checks.exports=true;
 const shapes=await page.evaluate(async()=>{
  ...
  for(const palette of ['bonus','counter']){
   const m=await fetch(`/fonts/roo-${palette}-v5.json`).then(r=>r.json()),image=new Image();image.src=`/fonts/roo-${palette}-v5.png`;await image.decode();
   ...
   for(const [char,g]of Object.entries(m.glyphs)){
    ...
    const override=m.outlineOverrides?.[char];
    if(override){
      const offset=1-g.inkTop-b[3]*sy,path=new Path2D();
      e.setTransform(1,0,0,1,0,0);e.clearRect(0,0,g.width,g.height);
      for(const c of override.commands){if(c.type==='M')path.moveTo(c.x,c.y);else if(c.type==='L')path.lineTo(c.x,c.y);else if(c.type==='Q')path.quadraticCurveTo(c.x1,c.y1,c.x,c.y);else if(c.type==='C')path.bezierCurveTo(c.x1,c.y1,c.x2,c.y2,c.x,c.y);else if(c.type==='Z')path.closePath();}
      e.setTransform(sx*cap,0,0,-sy*cap,-g.left*cap,(1-offset-g.top)*cap);e.fill(path);
    }
 ...
 for(const p of ['bonus','counter']){assert.equal(shapes[p].fontSha256,sha);assert.equal(shapes[p].version,5);assert.equal(shapes[p].pink,0);assert.ok(shapes[p].partial>5000);assert.ok(shapes[p].worst.iou>.96,JSON.stringify(shapes[p].worst));}
 ...
 report.checks.lightFrames=await page.evaluate(async()=>{
  const results={};
  for(const palette of ['bonus','counter']){
   const m=await fetch(`/fonts/roo-${palette}-v5.json`).then(r=>r.json()),frames=[];
   for(const frame of [0,1,2]){const image=new Image();image.src=`/fonts/roo-${palette}-v5${frame?'-light'+frame:''}.png`;await image.decode();const canvas=document.createElement('canvas');canvas.width=m.width;canvas.height=m.height;const ctx=canvas.getContext('2d');ctx.drawImage(image,0,0);frames.push(ctx.getImageData(0,0,m.width,m.height).data);}
 ...
 report.checks.crossfade=await page.evaluate(()=>{
  ...
  let maxAlphaError=0,changed=0;for(let i=0;i<images[0].length;i+=4){for(const frame of images)maxAlphaError=Math.max(maxAlphaError,Math.abs(frame[i+3]-images[2][i+3]));if(images[0][i+3]>128&&[0,1,2].some(c=>images[0][i+c]!==images[4][i+c]))changed++;}
  return{maxAlphaError,changed};
 });
 assert.ok(report.checks.crossfade.maxAlphaError<=1,'crossfade changes alpha at overlapping letter edges');assert.ok(report.checks.crossfade.changed>0);
 ...
 if(process.env.ROO_REVIEW_GAME!=='0')for(const lite of [true,false]){
  ...
  const urls=await game.locator('.game-hud-layer svg image').evaluateAll(images=>images.map(i=>i.getAttribute('href')));assert.ok(urls.length>0&&urls.every(s=>/-v5(?:-light[12])?\.png/.test(s)));
