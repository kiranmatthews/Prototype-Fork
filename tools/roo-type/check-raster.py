...
provenance=json.loads((fonts/'roo-font-v7-provenance.json').read_text())
...
for palette in ['counter','bonus']:
 m=json.loads((fonts/f'roo-{palette}-v7.json').read_text())
 assert m['version']==7 and m['capPixels']==512 and m['contourSource']=='model-artwork'
 assert m['width']<=8192 and m['height']<=8192
 frames=[np.asarray(Image.open(fonts/f'roo-{palette}-v7{s}.png').convert('RGBA')) for s in ['', '-light1','-light2']]
