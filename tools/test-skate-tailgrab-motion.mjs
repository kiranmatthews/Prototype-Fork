...
await withSkateRuntime(async({player:p,server,THREE,level,step})=>{
  ...
  for(const stance of (process.env.TAILGRAB_DEBUG?[Number(process.env.TAILGRAB_DEBUG)]:[-1,1]))for(const deck of (process.env.TAILGRAB_REVERSED?[Math.PI]:process.env.TAILGRAB_DEBUG?[0]:[0,Math.PI]))for(const [position,heading] of (process.env.TAILGRAB_DEBUG?walls.slice(0,1):walls)){
    ...
    for(let f=0;f<150;f++){
      ...
      step(input);frames++;p.group.updateMatrixWorld(true);assert.equal(p.isBailing,false,'Tailgrab bailed');
      ...
      if(contact){
        started=true;assert.equal(contact.kind,'tail');assert.equal(p.stance,stance);
        ...
        if(p.grabPose>.95){
          ...
          const palm=p.boardG.worldToLocal(at(contact.hand));if(!(palm.z<-s.deckTailLength*k*.9&&Math.abs(palm.x)<.05))fail({kind:'edge',f,palm:palm.toArray()});
          const tail=v(0,0,-1).transformDirection(p.boardG.matrixWorld);
          const backwardReach=at(`wrist-${side}`).sub(at(`shoulder-${side}`)).dot(tail);if(backwardReach<.10)fail({kind:'back arm',stance,deck,f,backwardReach});
          const seat=at('hips').sub(feet).dot(tail);if(seat>-.10)fail({kind:'seat',stance,deck,f,seat});
          ...
          if(hip<.55||Math.max(...knees)>1.50||waist<.60||waist>1.90)fail({kind:'posture',stance,deck,f,hip,knees,waist});
        }
  ...
  console.log('PASS native Tailgrab back-arm reach, seated hips, tail-tip grip, planted feet, clearance and clean release/landing.');
});
