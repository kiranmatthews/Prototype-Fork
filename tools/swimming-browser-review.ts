...
function place(deep=false){p.respawn(g.level,true,true);if(deep){p.pos.set(g.level.spawnPos.x,g.level.water.seaLevel-1.6,52);p.prevPos.copy(p.pos);p.state='air';p.grounded=false;p.vVel=0;}p.snapRenderInterpolation();}
...
add('Side view',()=>side=!side);add('Normal controls',()=>{mode='normal';side=false;});
...
g.camera.lookAt=(...args:any[])=>{
  if(side){g.camera.position.copy(p.renderPosition).add(new THREE.Vector3(8,3,1));look(p.renderPosition.clone().add(new THREE.Vector3(0,1.4,0)));}
  else look(...args);
};
...
function report(){
  ...
  status.textContent=JSON.stringify({mode,state:p.state,clip:p.animationClipHint,board:p.freeSkate,
    ...
    lives:p.lives},null,2);requestAnimationFrame(report);
}
report();
