...
await withSkateRuntime(async({player:p,server,THREE,level,step})=>{
  ...
  let cases=0,frames=0,vertices=0,heldFrames=0,minShorts=Infinity,minShoe=Infinity,minHead=Infinity,minHip=Infinity,minWaist=Infinity,maxKnee=0,maxFoot=0,maxHand=0,maxSpineStep=0,maxWaistStep=0,minPalmFacing=1,armHeadHits=0,legHeadHits=0;const errors=[];
  ...
  const armHitsHead=(a,b,radius=.035)=>{
    ...
    for(const offset of [-radius,0,radius]){
      ...
      if(ray.intersectObjects(heads,false).length)return true;
    }
    return false;
  };
  const fail=e=>{if(errors.length<8&&!errors.some(old=>old.kind===e.kind))errors.push(e);};
  for(const stance of (process.env.JAPAN_DEBUG?[Number(process.env.JAPAN_DEBUG)]:[-1,1]))for(const deck of (process.env.JAPAN_REVERSED?[Math.PI]:process.env.JAPAN_DEBUG?[0]:[0,Math.PI]))for(const [position,heading] of (process.env.JAPAN_DEBUG?walls.slice(0,1):walls)){
    ...
    for(let f=0;f<150;f++){
      const input=makeInput({jumpHeld:true,grabHeld:f>=2&&f<32,grabPressed:f===2,moveX:f===2?1:0,moveY:f===2?-1:0});
      step(input);frames++;p.group.updateMatrixWorld(true);assert.equal(p.isBailing,false,'Japan bailed');
      ...
      if(contact){
        started=true;assert.equal(contact.kind,'japan');assert.equal(p.stance,stance);
        ...
        if(p.grabPose>.95){
          held++;heldFrames++;
          const side=stance>0?'right':'left';assert.equal(contact.hand,`socket-grip-${side}`);
          ...
          for(const limbSide of ['left','right']){
            for(const part of ['upper','lower'])assert.ok(p.playerAnimationBridge.deformationValue(`deform.leg.${part}.${limbSide}.length`)>.90,'Japan must retain leg length');
          }
          ...
          const inward=v(-stance*(deck?-1:1),0,0).transformDirection(p.boardG.matrixWorld);
          ...
          if(backReach>-.12)fail({kind:'free arm',stance,deck,f,backReach});
          for(const limbSide of ['left','right']){
            const knee=at(`knee-${limbSide}`),ankle=at(`ankle-${limbSide}`),hipJoint=at(`hip-${limbSide}`);
            const forward=knee.clone().sub(ankle).dot(toe),down=hipJoint.sub(knee).dot(v(0,1,0).transformDirection(p.bodyGroup.matrixWorld));
            if(forward<.25||down<.22)fail({kind:'knee direction',stance,deck,f,forward,down});
          }
          const seat=at('hips').sub(feet).dot(toe);if(seat<.35)fail({kind:'seat',stance,deck,f,seat});
          const shoulders=at('shoulder-left').add(at('shoulder-right')).multiplyScalar(.5);
          const waist=shoulders.sub(at('spine')).angleTo(v(0,1,0).transformDirection(p.bodyGroup.matrixWorld));minWaist=Math.min(minWaist,waist);
          if(hip<.35||Math.max(...knees)>2.75||waist>1.30)fail({kind:'posture',stance,deck,f,hip,knees,waist});
        }
