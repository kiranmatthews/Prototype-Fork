...
await withSkateRuntime(async({player:p,server,THREE,level,step})=>{
  ...
  for(const stance of (process.env.STALEFISH_DEBUG?[Number(process.env.STALEFISH_DEBUG)]:[-1,1]))for(const deck of (process.env.STALEFISH_REVERSED?[Math.PI]:process.env.STALEFISH_DEBUG?[0]:[0,Math.PI]))for(const [position,heading] of (process.env.STALEFISH_DEBUG?walls.slice(0,1):walls)){
    ...
    for(let f=0;f<150;f++){
      ...
      step(input);frames++;p.group.updateMatrixWorld(true);assert.equal(p.isBailing,false,'Stalefish bailed');
      ...
      if(contact){
        started=true;assert.equal(contact.kind,'stalefish');assert.equal(p.stance,stance);
        ...
        if(p.grabPose>.95){
          held++;heldFrames++;const side=stance>0?'left':'right';assert.equal(contact.hand,`socket-grip-${side}`);
          ...
          for(const limbSide of ['left','right']){
            for(const part of ['upper','lower'])assert.ok(p.playerAnimationBridge.deformationValue(`deform.leg.${part}.${limbSide}.length`)>.90,'Stalefish must retain leg length');
          }
          ...
          const toe=v(0,0,1).transformDirection(p.bodyGroup.matrixWorld),freeSide=stance>0?'right':'left';
          ...
          if(hip<.55||Math.max(...knees)>1.45||waist<.65||waist>1.95)fail({kind:'posture',stance,deck,f,hip,knees,waist});
        }
  ...
  console.log('PASS native Stalefish seated hips, side fold, clear limbs and trailing heel-edge grip, planted feet, clearance and clean release/landing.');
});
