...
await withSkateRuntime(async({player:p,THREE,server})=>{
  ...
  const selected=catalog.clips.filter(c=>/Skate · S(?:09|1[0-9]|2[0-2]|2[6-9]|3[0-8]|42) ·/.test(c.name));assert.equal(selected.length,28);
  ...
  for(const clip of selected){
    ...
    const wrapped=clip.metadata.skateReviewId==='flip:imposs',indy=clip.metadata.skateReviewId==='grab:indy',melon=clip.metadata.skateReviewId==='grab:melon',nosegrab=clip.metadata.skateReviewId==='grab:nose',tailgrab=clip.metadata.skateReviewId==='grab:tail',method=clip.metadata.skateReviewId==='grab:method';
    ...
    for(let f=0;f<=120;f++){
      ...
      if(indy||melon||endGrab||method){
        ...
        if(state?.grabWeight>.95){
          ...
          assert.ok(at('hips').sub(feet).dot(normal)>(melon||endGrab||method?.55:.75),'captured grab squats below supported reach');
          const settings=p.boardG.userData.settings,scale=settings.overallScale,parity=Math.cos(state.deckYaw)<0?-1:1;
          const x=endGrab?0:settings.deckHalfWidth*scale*state.stance*parity*(melon||method?-1:1),z=nosegrab?settings.deckNoseLength*scale*.96:tailgrab?-settings.deckTailLength*scale*.96:(melon||method?.10:-.10)*parity;
          ...
          const side=endGrab?(state.stance*parity*(nosegrab?1:-1)>0?'right':'left'):(state.stance>0)!==(melon||method)?'left':'right';
          ...
          for(const side of ['left','right']){const knee=at(`knee-${side}`).sub(at(`hip-${side}`)).angleTo(at(`ankle-${side}`).sub(at(`knee-${side}`)));assert.ok(knee<(endGrab||method?1.50:melon?1.45:1.20),`${clip.name} knee ${side} bends ${knee*180/Math.PI} degrees at ${time}`);}
        }
      }
  ...
  console.log('PASS S09–S22/S26–S38/S42 captured entry, trick and exit garment/board and rail clearance.');
});
