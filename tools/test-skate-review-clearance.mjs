...
await withSkateRuntime(async({player:p,THREE,server})=>{
  ...
  const selected=catalog.clips.filter(c=>/Skate · S(?:09|1[0-9]|2[0-5]|2[6-9]|3[0-8]|42) ·/.test(c.name));assert.equal(selected.length,31);
  ...
  for(const clip of selected){
    ...
    for(let f=0;f<=120;f++){
      ...
      if(indy||melon||endGrab||method||mute||stalefish||japan){
  ...
  console.log('PASS S09–S38/S42 captured entry, trick and exit garment/board and rail clearance.');
});
