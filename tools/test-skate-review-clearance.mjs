...
await withSkateRuntime(async({player:p,THREE,server})=>{
  ...
  const selected=catalog.clips.filter(c=>/Skate · S(?:09|1[0-9]|2[0-4]|2[6-9]|3[0-8]|42) ·/.test(c.name));assert.equal(selected.length,30);
  ...
  for(const clip of selected){
    ...
    for(let f=0;f<=120;f++){
      ...
      if(indy||melon||endGrab||method||mute||stalefish){
        ...
        if(state?.grabWeight>.95){
          ...
          assert.ok(at('hips').sub(feet).dot(normal)>(melon||endGrab||method||mute||stalefish?.55:.75),'captured grab squats below supported reach');
          const settings=p.boardG.userData.settings,scale=settings.overallScale,parity=Math.cos(state.deckYaw)<0?-1:1;
          const x=endGrab?0:settings.deckHalfWidth*scale*state.stance*parity*(melon||method||stalefish?-1:1),z=nosegrab?settings.deckNoseLength*scale*.96:tailgrab?-settings.deckTailLength*scale*.96:(melon||method||mute?.10:-.10)*parity;
  ...
  console.log('PASS S09–S24/S26–S38/S42 captured entry, trick and exit garment/board and rail clearance.');
});
