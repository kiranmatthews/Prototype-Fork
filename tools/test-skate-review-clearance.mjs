...
await withSkateRuntime(async({player:p,THREE,server})=>{
  ...
  const selected=catalog.clips.filter(c=>/Skate · S(?:09|1[0-8]|2[6-9]|3[0-8]|42) ·/.test(c.name));assert.equal(selected.length,24);
  ...
  for(const clip of selected){
    ...
    const wrapped=clip.metadata.skateReviewId==='flip:imposs',indy=clip.metadata.skateReviewId==='grab:indy';
    ...
    for(let f=0;f<=120;f++){
      ...
      if(indy){
        ...
        if(state?.grabWeight>.95){
          ...
          assert.ok(at('hips').sub(feet).dot(normal)>.75,'captured Indy squats below standing reach');
          const settings=p.boardG.userData.settings,scale=settings.overallScale,parity=Math.cos(state.deckYaw)<0?-1:1;
          const x=settings.deckHalfWidth*scale*state.stance*parity,z=-.10*parity;
          ...
          const gripError=at(`socket-grip-${state.stance>0?'left':'right'}`).distanceTo(target);assert.ok(gripError<.008,`captured Indy loses its toe-edge grip at ${time}: ${gripError}; weight ${state.grabWeight}`);
          for(const side of ['left','right'])assert.ok(at(`knee-${side}`).sub(at(`hip-${side}`)).angleTo(at(`ankle-${side}`).sub(at(`knee-${side}`)))<1.20,'captured Indy knee folds too far');
        }
      }
  ...
  console.log('PASS S09–S18/S26–S38/S42 captured entry, trick and exit garment/board and rail clearance.');
});
