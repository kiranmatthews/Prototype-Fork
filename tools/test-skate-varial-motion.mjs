...
await withSkateRuntime(async({player:p,server,THREE,Level,Player})=>{
  ...
  const level=new Level(new THREE.Scene(),{id:'varial-test',name:'Varial test',data:{v:1,name:'Varial test',spawn:[0,.02,0],killY:-30,
  ...
  let cases=0,frames=0,vertices=0,minShorts=Infinity,minShoe=Infinity,minShin=Infinity,maxFoot=0,maxArc=0,maxSpan=0,maxYawError=0,maxRollError=0;const failures=[];
  ...
  for(const kind of ['varial','varial-heel'])for(const park of (process.env.VARIAL_DEBUG?[false]:[false,true]))for(const stance of (process.env.VARIAL_DEBUG?[-1]:[-1,1]))for(const reversed of (process.env.VARIAL_DEBUG?[false]:[false,true]))for(const delay of (process.env.VARIAL_DEBUG?[1]:[1,9])){
    cases++;level.skatepark=park;const sign=kind==='varial'?1:-1,parity=reversed?-1:1;
    for(const rider of [p,control]){
      rider.enterLevel('varial-test');rider.respawn(level,true);rider.parkControls=park;rider.freeSkate=true;rider.speed=8;rider.parkVelocity.set(0,0,-8);
    ...
    runtime.restart();let air=0,started=false,landAge=0,scissor=false,tailLoad=false,balanced=false,initialNose=null,initialBody=null,lastYaw=null,lastRoll=null,yawTotal=0,rollTotal=0;const catches=[];
    for(let f=0;f<230;f++){
      if(p.state==='air')air++;
      const input=makeInput({jumpHeld:f>=30&&f<60,jumpPressed:f===30,jumpReleased:f===60,spinPressed:air===delay,spinHeld:air===delay,moveX:air===delay?-sign:0,moveY:air===delay?-1:0});
      p.step(1/60,input,level);control.step(1/60,input,level);level.update(1/60);frames++;p.group.updateMatrixWorld(true);
      assert.ok(p.pos.distanceTo(control.pos)<1e-9,'varial animation changed movement');assert.equal(p.isBailing,false);
      ...
      if(m){
        ...
        if(t>.16&&t<.35&&front.clone().sub(centre).dot(right)*stance*sign>.1&&back.clone().sub(centre).dot(right)*stance*sign<-.1)scissor=true;
        if(t>.18){
        ...
        if(m.arms>.95&&span>1.25&&frontReach.dot(up)-backReach.dot(up)>.1&&frontReach.length()-backReach.length()>.05)balanced=true;
    ...
    if(!(started&&scissor&&tailLoad&&balanced&&landAge>20))fail({kind:'phases',trick:kind,park,stance,reversed,delay,started,scissor,tailLoad,balanced,landAge});
    maxYawError=Math.max(maxYawError,Math.abs(yawTotal-Math.PI*sign*stance));maxRollError=Math.max(maxRollError,Math.abs(rollTotal+2*Math.PI*sign*stance*parity));
    assert.ok(Math.abs(yawTotal-Math.PI*sign*stance)<.02,`${kind} misses its half-shove`);assert.ok(Math.abs(rollTotal+2*Math.PI*sign*stance*parity)<.02,`${kind} misses its complete roll`);
    ...
    assert.equal(p.comboPoints,kind==='varial'?300:350);assert.equal(p.comboMult,1);
  }
  runtime.dispose();level.dispose();console.log({cases,frames,vertices,minShorts,minShoe,minShin,maxFoot,maxArc,maxSpan,maxYawError,maxRollError,failures});assert.deepEqual(failures,[]);
  console.log('PASS native opposing scoop/flick, combined half-shove/full roll, asymmetric arm balance, independent flight, downward catch, mesh clearance and unchanged movement.');
});
