// Browser proof for full-precision CRT decode + OutputPass: graph texels, alpha
...
assert.ok(candidate,'Usage: node tools/test-crt-output-browser.mjs <candidate-url> [baseline-url]');
const output=process.env.CRT_OUTPUT_REVIEW||'/private/tmp/crt-output-review';
...
try{
  ...
  report.fusion=await page.evaluate(async()=>{
    ...
    const deferred=new CrtGuestPass(renderer,settings,{luts,respectDisableQuery:false,deferOutput:true});
    const standard=new OutputPass(),fused=new CrtGuestOutputPass(()=>deferred.deferredOutput);
    assert(deferred.needsSwap===false,'Deferred CRT must preserve the composer read buffer');
    assert(deferred.deferredOutput===null,'Unrendered CRT must not expose a guest texture');
    ...
    try{
      ...
      for(const [scenario,spec] of specs.entries()){
        ...
        settings.setVariant(spec.variant);settings.setValue('oimage',spec.original);
        for(const pass of [reference,deferred])pass.setResolution(source.width,source.height,decoded.width,decoded.height);
        assert(deferred.deferredOutput===null,'Resolution/settings changes must invalidate a previously completed guest output');
      ...
      const checkFallback=async(label)=>{
        assert(deferred.deferredOutput===null,`${label}: stale guest texture was exposed before rendering`);
        renderer.info.reset();deferred.render(renderer,unused,source,1/60,false);
        assert(deferred.deferredOutput===null,`${label}: inactive graph exposed a guest texture`);
      ...
      settings.setEnabled(true);deferred.render(renderer,unused,source,1/60,false);assert(deferred.deferredOutput,'Re-enable failed');
      ...
      deferred.retryAfterFailure();deferred.render(renderer,unused,source,1/60,false);assert(deferred.deferredOutput,'Explicit graph retry failed');
      let borrowedDisposed=0;const borrowed=deferred.targets.deconvergence;
      ...
      assert(deferred.deferredOutput===null,'Disposed CRT must not expose guest output');
