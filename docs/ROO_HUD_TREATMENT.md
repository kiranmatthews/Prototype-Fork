...
Six live sliders in **M → Text Tuning → Menu PNG focus filters** adjust rate, white duration, white brightness, white desaturation, inactive saturation and inactive brightness. Default inactive treatment changes saturation only. A 100% white duration holds the white preview; zero rate holds the normal orange image. Reset restores the measured timing and neutral-PNG defaults. Controls persist in fork-owned `solProtoMenuPngFocusV2` storage and stay out of gameplay menus.

Reproduce measurements with `python3 tools/roo-type/measure-menu-reference.py /path/to/reference.mp4 --ffmpeg /path/to/ffmpeg --output /tmp/reference.json`. The video itself is not vendored. `tools/test-menu-png-focus.mjs` compares sixty displayed frames to the measured pattern, checks white intensity, original PNG source/alpha/markup, controller focus, all six sliders, reduced motion and idle behaviour in lite/full rendering.
