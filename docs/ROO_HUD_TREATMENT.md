...
Menu action labels have a separate, reversible runtime ink layer. Their focused state alternates the original orange artwork with bright white shading on a 900 ms cycle; inactive labels use a brown-tan matrix. Canvas word caches and native SVG filters use the same sRGB matrices and retain alpha. Menu labels use the neutral source frame so glisten cannot interfere with focus readability; HUD/title artwork and the v9 PNGs remain unchanged. The focus clock is independent of shimmer, stops when menus close, and holds orange for reduced motion. **Menu focus colours (orange / white)** in the M-dismissible Text Tuning panel restores the previous treatment when unchecked.
