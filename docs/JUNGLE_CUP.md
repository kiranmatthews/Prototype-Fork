...
The introduction's **TRICK GUIDE** lists the eight directional flips, eight grabs, grind styles and SPECIAL commands, with their base points. Square/F flips work throughout vert air and can buffer through a catch or grab release. Grab entry direction remains fixed while the rider rotates. Landed combos share trick depreciation for the run (100%, 75%, 50%, 25%, then 10%); duration points obey the same rule. See [TRICK_SYSTEM_AUDIT.md](TRICK_SYSTEM_AUDIT.md) for the source audit, complete controls, animation behavior and scoring examples.
...
Flat park travel now uses the main game's live camera height, distance, pitch and field of view (defaults: 5.1 m, 5.05 m, 25.35° and 49°, plus the existing speed lens effect). It still follows behind the rider. A surface-slope blend hands back to the calibrated skate shot between 5° and 35°; steep transitions and every vert air use the original shot exactly. Only the final camera presentation blends: the underlying vert orientation, pivot smoothing, landing timing and player-relative controls remain unchanged. The return to flat framing eases in and retains camera collision correction.
...
`npm run check:competition` covers the event model and the real Level/Player runtime. Skating checks include all sides/corners at three speeds, measured gravity/impulses, true lip launches, curved tracking, landing contact, 19,200 mixed-input frames and all 3,117 frames of the supplied replay. The layout suite adds exact feature heights, outside-bank serialization, nondegenerate surfaces, pocket airs, smooth roll-in, channel traversal, a rhythm jump/grind sequence and both directions on the new rails. Camera checks include complete rider framing, visibility through coping, wall-oriented apex and 30/60/120 Hz equivalence. Event coverage includes 100 simulated minutes of overtime, final bank/bail/same-tick closure, reset/pause, real manual linking, pending flips, and automatic ground/air remounting. Follow-up regressions cover 16 actual rail catches, 553 grind frames and 14 exits, plus 49 deck/corner/lip/air/standable-bank bail cases without input; all recover within 1.73 seconds. Geometry checks cover the actual vertical top, continuous banks, foundation, containment at three heights, rail scoring and editor round trips.

For browser review, run Vite and open `/jungle-cup-review.html?lite&playtest&level=jungle-cup`. The local-only review controls run the real game loop through each wall, corners, a carve sequence, the pocket bowl, rhythm line, recovery cases and the attached replay, with an optional pause at the apex. Park overview provides a local-only layout inspection shot. Remove `lite` for the full renderer. This QA entry and its scripted controls are not part of the published build.

`npm run check:park-camera` is the focused framing/grind check. It compares the main-game flat pose at multiple headings and settings, proves unchanged eye/orientation/lens across 163 steep/vert samples, checks smooth recovery and 30/60/120 Hz equivalence, and rides four real perimeter routes plus the existing rail catches. `/park-camera-review.html?playtest&level=jungle-cup` provides local-only old/new flat comparison and south/east vert runs with an apex freeze; its DOM status exposes the presented blend and camera pose without changing stored tuning or completing the event.

`tools/test-jungle-cup-ui.mjs` exercises selection/focus, four-way navigation, held-input edges, press feedback, suppressed-menu handoff, disabled judges and the single in-run points display. `tools/test-hud-visibility.mjs` covers persistent competition score/avatar and inventory-mode transitions. `/competition-review.html?playtest&level=jungle-cup&ui=intro` provides local-only phase fixtures and controller buttons, with compositor diagnostics; `ui` also accepts `countdown`, `running`, `overtime`, `judges`, `standings`, `win` and `loss`. Fixture actions are recorded instead of committing campaign results.
