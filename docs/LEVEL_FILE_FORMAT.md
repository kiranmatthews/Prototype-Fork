...
A `thorn` is visual warning artwork with a base anchor `p`, nominal `s` dimensions
(default `[2.14, 1.01, 2.26]`), `yaw`, glow `color`, and optional deterministic
`seed`. Its inspector and handles edit those values without inserting defaults
on selection. Clearing the seed restores the original variation. A separate,
