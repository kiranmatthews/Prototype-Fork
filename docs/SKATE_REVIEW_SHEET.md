...
Keys are reduced against the 30 fps capture (60 fps for lip stalls, Kickflip, Heelflip, Pop Shove-It, Backflip and the under-rail hang) with 0.0002 local-unit/scalar tolerance and 0.0009 radian quaternion tolerance, then written to six decimals. S13–S16 use 240 Hz native stepping with 60 Hz input edges, and tighter 0.00005 position/scalar and 0.0002-radian quaternion tolerances to preserve the rolling shoe contact and combined rotations. The 42-clip file is about 9.1 MiB before HTTP compression, fetched only by the sheet or when opening the Lab. The renderer reuses the same character surfaces; it does not allocate a WebGL context per card.
